    ...
    <div class="form-panel">
        <div class="form-page-header">
            <div>
                ...
                <p class="mt-2 text-sm text-black/65">{{ __('Move someone into leadership, update their church location, or return them to normal member access from here.') }}</p>
            </div>
        ...
        <form class="mt-6 form-stack" method="POST" action="{{ route('admin.users.update', $managedUser) }}">
            ...
            <div class="form-actions pt-2">
                <button class="btn-rgc w-full sm:w-auto" type="submit">@include('partials.ui.icon', ['name' => 'edit', 'class' => 'button-icon'])<span>{{ __('Save changes') }}</span></button>
            </div>
        </form>
    </div>
</div>
