...
@section('title', __('Ongeza Sadaka - Mfumo wa Kanisa'))
@section('page-title', __('Ongeza Sadaka Mpya'))
@section('page-subtitle', __('Jaza fomu ili kuongeza rekodi mpya ya sadaka'))
...
<div class="space-y-6">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">{{ __('Ongeza Sadaka Mpya') }}</h1>
            <p class="text-gray-600 mt-2">{{ __('Jaza taarifa za sadaka mpya') }}</p>
        </div>
        <div class="flex flex-wrap gap-3">
            <a href="{{ route('offerings.index') }}" class="px-5 py-2.5 bg-gray-200 text-gray-700 rounded-xl transition-all duration-200 flex items-center gap-2 hover:bg-gray-300">
                ...
                <span class="font-medium">{{ __('Rudi Nyuma') }}</span>
            </a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div class="p-5 bg-gradient-to-r from-primary-600 to-primary-700">
            <h3 class="text-lg font-semibold text-white flex items-center">
                <i class="fas fa-hand-holding-heart mr-2"></i> {{ __('Taarifa za Sadaka') }}
            </h3>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label for="collection_date" class="block text-sm font-medium text-gray-700 mb-2">
                        <i class="fas fa-calendar-alt text-primary-500 mr-1"></i> {{ __('Tarehe ya Kukusanya') }}
                    </label>
                    <input type="date"
                           id="collection_date"
                ...
                <div>
                    <label for="income_category_id" class="block text-sm font-medium text-gray-700 mb-2">
                        <i class="fas fa-tag text-primary-500 mr-1"></i> {{ __('Aina ya Sadaka') }}
                    </label>
                ...
                <div>
                    <label for="amount" class="block text-sm font-medium text-gray-700 mb-2">
                        <i class="fas fa-money-bill-wave text-primary-500 mr-1"></i> {{ __('Kiasi (TZS)') }}
                    </label>
                    <input type="number"
                           id="amount"
                           name="amount"
                ...
                <div>
                    <label for="member_id" class="block text-sm font-medium text-gray-700 mb-2">
                        <i class="fas fa-user text-primary-500 mr-1"></i> {{ __('Muumini (Hiari)') }}
                    </label>
                    <select id="member_id"
                            name="member_id"
                ...
                <div class="md:col-span-2">
                    <label for="notes" class="block text-sm font-medium text-gray-700 mb-2">
                        <i class="fas fa-sticky-note text-primary-500 mr-1"></i> {{ __('Maelezo (Hiari)') }}
                    </label>
                    <textarea id="notes"
                              name="notes"
                              rows="3"
                              placeholder="{{ __('Maelezo ya ziada kuhusu sadaka hii...') }}"
